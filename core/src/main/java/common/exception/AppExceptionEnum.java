...
public enum AppExceptionEnum {
	...
	public static AppExceptionEnum getByCode(String code){
		List<AppExceptionEnum> list =  Arrays.stream(AppExceptionEnum.values())
				.filter((e)->{return StringUtils.equals(e.getCode(), code);})
				.collect(Collectors.toList());
		return Optional.ofNullable(list).isPresent()&&list.size()>0?list.get(0):SystemError;
	}
}
